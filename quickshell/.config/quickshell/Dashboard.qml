...
PopupWindow {
    ...
    // PopupSurface carries the fill, the border and the open animation.
    // Its 0.98 alpha is not a style choice: this is an xdg_popup, not a
    // layer surface, so no hyprland layerrule blurs what is behind it — at
    // 0.94 a bright terminal underneath showed its text through the
    // calendar.
    PopupSurface {
        anchors.fill: parent
        shown: dash.visible
